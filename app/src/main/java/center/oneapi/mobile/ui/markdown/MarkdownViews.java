...
public class MarkdownViews {
    ...
    private View thinking(String value, Runnable clickAction) {
        ...
        box.setBackground(UiKit.round(Color.argb(58, 190, 205, 230), UiKit.dp(context, 12), Color.argb(92, 120, 145, 190)));
        ...
        TextView title = UiKit.text(context, "Thinking", Color.rgb(70, 88, 120), 13);
        title.setTypeface(Typeface.DEFAULT_BOLD);
        head.addView(title, new LinearLayout.LayoutParams(-2, UiKit.dp(context, 28)));
        TextView marker = UiKit.text(context, "▶", Color.rgb(70, 88, 120), 7);
        ...
        TextView body = UiKit.text(context, value, Color.rgb(84, 99, 128), 13);
    ...
    private View code(String language, String value, Runnable clickAction) {
        LinearLayout box = UiKit.vertical(context);
        box.setBackground(UiKit.round(Color.rgb(244, 247, 251), UiKit.dp(context, 10), Color.rgb(220, 228, 241)));
        LinearLayout top = UiKit.horizontal(context);
        top.setPadding(UiKit.dp(context, 10), UiKit.dp(context, 5), UiKit.dp(context, 6), 0);
        TextView lang = UiKit.text(context, language == null || language.isEmpty() ? "code" : language, Color.rgb(92, 105, 128), 12);
        ...
        copy.setColorFilter(UiKit.MUTED);
        ...
        box.addView(top);
        TextView view = UiKit.text(context, value, Color.rgb(31, 41, 55), 12);
    ...
    private View mermaid(String value, Runnable clickAction) {
        ...
        TextView label = UiKit.text(context, "mermaid", Color.rgb(92, 105, 128), 12);
        ...
        svg.setColorFilter(UiKit.MUTED);
    ...
    private View quote(String value, Runnable clickAction) {
        TextView view = UiKit.text(context, value, Color.rgb(70, 88, 120), 14);
        view.setText(styled(value));
        view.setTextIsSelectable(true);
        view.setBackground(UiKit.round(Color.argb(28, 54, 104, 240), UiKit.dp(context, 10), Color.argb(52, 54, 104, 240)));
    ...
    private View tableRow(List<String> cells, boolean header, Runnable clickAction) {
        ...
        for (String cell : cells) {
            ...
            view.setBackground(UiKit.round(header ? Color.rgb(247, 249, 255) : Color.argb(180, 255, 255, 255), UiKit.dp(context, 8), UiKit.LINE));
            if (header) view.setTypeface(Typeface.DEFAULT_BOLD);
            if (clickAction != null) view.setOnClickListener(v -> clickAction.run());
            TableRow.LayoutParams lp = new TableRow.LayoutParams(-2, -2);
            lp.setMargins(0, 0, UiKit.dp(context, 4), UiKit.dp(context, 4));
            row.addView(view, lp);
        }
        return row;
    }
    ...
    private CharSequence styled(String source) {
        ...
        for (int[] span : codeSpans) {
            if (span[1] > span[0]) {
                ...
                styled.setSpan(new BackgroundColorSpan(Color.rgb(238, 242, 248)), span[0], span[1], Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
                styled.setSpan(new ForegroundColorSpan(Color.rgb(42, 52, 70)), span[0], span[1], Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
        return styled;
    }
    ...
    private String mermaidHtml(String source) {
        return "<!doctype html><html><head><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"/>"
                ...
                + "<style>body{margin:0;background:transparent;font-family:sans-serif}.mermaid{padding:12px;overflow:auto}</style>"
                + "</head><body><pre class=\"mermaid\">" + escapeXml(source) + "</pre>"
                + "<script>mermaid.initialize({startOnLoad:true,securityLevel:'loose',theme:'default'});</script></body></html>";
    }
